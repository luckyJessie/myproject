...
class FileUploader {
  ...
  mkFile (fileItem, ctx) {
    ...
    xhr.setRequestHeader('Authorization', 'UpToken ' + fileItem.token)
    xhr.onload = () => {
      // console.log(xhr)
  ...
  simpleUpload (fileItem) {
    ...
    xhr.upload.addEventListener('progress', function (evt) {
      if (evt.lengthComputable) {
        ...
        tmp = evt.loaded
        progress.stTime = parseFloat(((fileItem.size - tmp) / (1024 * uploadSpeed)).toFixed(1))
        progress.percent = Math.round(100 * tmp / fileItem.size) > 100 ? 100 : Math.round(100 * tmp / fileItem.size)
        // console.log(100 * tmp / fileItem.size, progress.percent, progress.speed, progress.stTime)
        uploader._onProgressUpload(fileItem, progress)
      }
    }, false)
    // 上传完成
