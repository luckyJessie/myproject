...
<template>
  <div class="screenshot">
    ...
    <div class="controls">
      <input type="file"accept="image/*"@change="imgChange"style="display:none"class="pic">
      <button @click="selectImg">选择图片</button>
      <select name="type" v-model="type" :disabled="!typeChange">
        <option value="free">自由</option>
        <option value="fixedScale">等比例</option>
        <option value="fixedSize">固定尺寸</option>
      </select>
      <span class="cutPre-block" v-if="type==='fixedScale' || type==='fixedSize'">
        W: <input type="number" v-model.number="cutPre.width">
         -
        H: <input type="number" v-model.number="cutPre.height">
      </span>
      <button @click="recut" v-if="status ==='completed'">重新剪切</button>
      <a v-if="status==='completed'" :href="imgdest.url" download="未命名.png">下载到本地</a>
    </div>
  </div>
</template>
...
<script>
  export default {
    ...
    methods: {
      selectImg () {
        var input = document.getElementsByClassName('pic')[0]
        input.click()
      },
      initImgsrc (img) {
